...
// Description:
//
// G4MOSSNuclearLevelManager is the class used to import nuclear level data
// from databases and build instantiations of G4MOSSNuclearLevel.
//
//
...
//    bool gets passed from G4MOSS.cc, and if true, disables some error checking
...
//    uses G4MOSSNuclearLevel::PrintAllTabular() to print everything to an ofstream
//    file that also gets set in G4MOSS.cc.
//
// -------------------------------------------------------------------
...
G4MOSSNuclearLevelManager::G4MOSSNuclearLevelManager(G4bool Verbose):
  _nucleusA(0), _nucleusZ(0), _fileName(""), _validity(false),
  _levels(0), _levelEnergy(0), _gammaEnergy(0), _probability(0),
  _Verbose(Verbose) { }

G4MOSSNuclearLevelManager::G4MOSSNuclearLevelManager(const G4int Z, const G4int A, const G4String& filename, G4bool Verbose) :
  _nucleusA(A), _nucleusZ(Z), _fileName(filename), _Verbose(Verbose) {
  if (A <= 0 || Z <= 0 || Z > A)
    throw G4HadronicException(__FILE__, __LINE__, "==== G4MOSSNuclearLevelManager ==== (Z, A) < 0, or Z > A");

  _levels = 0;

  if (_Verbose) {
    G4cout << "G4MOSSNuclearLevelManager: Initializing (Z, A) = ("
     << Z << ","
     << A << ")" << G4endl;
  }
...
G4MOSSNuclearLevelManager::~G4MOSSNuclearLevelManager() {
  if (_levels) {
    if (_levels->size() > 0) {
...
void G4MOSSNuclearLevelManager::SetNucleus(const G4int Z, const G4int A, const G4String& filename, G4bool standalone) {
  if (_nucleusZ != Z || _nucleusA != A) {
    ...
    if (_Verbose) {
      G4cout << "G4MOSSNuclearLevelManager::SetNucleus Initializing (Z, A) = ("
             << Z << ","
             << A << ")" << G4endl;

      G4cout << "G4MOSSNuclearLevelManager::SetNucleus Calling ReadGroundStateProperties"
             << G4endl;
    }

    if (_Verbose) {
      G4cout << "G4MOSSNuclearLevelManager::SetNucleus Calling MakeLevels"
             << G4endl;
    }
    ...
    if (_Verbose) {
      G4cout << "G4MOSSNuclearLevelManager::SetNucleus Back from MakeLevels" << G4endl;
    }
  }
}

void G4MOSSNuclearLevelManager::ReadGroundStateProperties(G4bool standalone) {
  // NRF-specific routine
  //
  // Reads spin and parity of ground state from file
  // $G4MOSSGAMMADATA/ground_state_properties.dat

  char* env = getenv("G4MOSSGAMMADATA");
  if (!env) {
    G4cout << "G4MOSSNuclearLevelManager: please set the G4MOSSGAMMADATA environment variable"
           << G4endl;
  ...
  if (!gsFile) {
    G4cout << "Error in G4MOSSNuclearLevelManager::ReadGroundStateProperties." << G4endl;
  ...
  while ((!found_gs) && (!gsFile.eof())) {
    ...
    if ((Z == _nucleusZ) && (A == _nucleusA)) {
      ...
      if (spin >= 0.0) {
        ...
        if (_Verbose) {
          G4cout << "G4MOSSNuclearLevelManager::ReadGroundStateProperties -- Found valid g.s. spin = "
                 << spin << G4endl;
        }
      } else {
        G4cout << "WARNING from G4MOSSNuclearLevelManager::ReadGroundStateProperties."
               << G4endl;
      ...
      if (parity != 0.0) {
        _gsParity = parity;
        if (_Verbose) {
          G4cout << "G4MOSSNuclearLevelManager::ReadGroundStateProperties -- Found valid g.s. parity = "
                 << parity << G4endl;
        }
      } else {
        G4cout << "WARNING from G4MOSSNuclearLevelManager::ReadGroundStateProperties."
...
void G4MOSSNuclearLevelManager::ReadTDebyeData(G4bool standalone) {
  char* env = getenv("G4MOSSGAMMADATA");
  if (!env) {
    G4cout << "G4MOSSNuclearLevelManager: please set the G4MOSSGAMMADATA environment variable" << G4endl;
    G4cout << "Aborting." << G4endl;
    exit(5);
  }
  ...
  if (!found_TD) {
    G4cout << "ERROR in G4MOSSNuclearLevelManager::ReadTDebyeData." << G4endl;
...
void G4MOSSNuclearLevelManager::SetTDebye(G4double TDebye) {
  _TDebye = TDebye;
}

G4double G4MOSSNuclearLevelManager::GetTDebye() const {
  return _TDebye;
}

G4double G4MOSSNuclearLevelManager::GetGroundStateSpin() const {
  return _gsAngularMomentum;
}

G4double G4MOSSNuclearLevelManager::GetGroundStateParity() const {
  return _gsParity;
}

G4double G4MOSSNuclearLevelManager::GetTotalCC() const{//added in feb. 10th, 2022
  return _kCC;
}

G4bool G4MOSSNuclearLevelManager::IsValid() const {
  return _validity;
}

G4int G4MOSSNuclearLevelManager::NumberOfLevels() const {
  G4int n = 0;
  if (_levels != 0) n = _levels->size();
  return n;
}

const G4MOSSPtrLevelVector* G4MOSSNuclearLevelManager::GetLevels() const {
  return _levels;
}


G4MOSSNuclearLevel* G4MOSSNuclearLevelManager::
NearestLevelRecoilAbsorb(const G4double GammaEnergy, const G4double eDiffMax) {
  ...
  const G4double E_recoil = GammaEnergy*GammaEnergy/(2.0 * M); // see G4MOSS class for factor of 2 explanation

  G4double E_internal_excite = GammaEnergy - E_recoil;

  if (_Verbose) {
    G4cout << "G4MOSSNuclearLevelManager::NearestLevelRecoilAbsorb -- " << G4endl;
...
G4double G4MOSSNuclearLevelManager::MinLevelEnergy() const {
...
G4double G4MOSSNuclearLevelManager::MaxLevelEnergy() const {
  G4double eMax = 0.;
  if (_levels != 0) {
...
const G4MOSSNuclearLevel* G4MOSSNuclearLevelManager::HighestLevel() const {
...
const G4MOSSNuclearLevel* G4MOSSNuclearLevelManager::LowestLevel() const {
  if (_levels != 0 && _levels->size() > 0)
    return _levels->back();
  else
    return 0;
...
G4bool G4MOSSNuclearLevelManager::Read(std::ifstream& dataFile) {
...
void G4MOSSNuclearLevelManager::MakeLevels() {
  _validity = false;

  std::ifstream inFile(_fileName, std::ios::in);
  if (!inFile) {
    if (_nucleusZ > 10) G4cout << " G4MOSSNuclearLevelManager: nuclide ("
                               << _nucleusZ << "," << _nucleusA
  ...
  if (_levels != 0) {
    if (_levels->size() > 0) {
      std::vector<G4MOSSNuclearLevel*>::iterator pos;
  ...
  _levels = new G4MOSSPtrLevelVector;
  ...
  for (i = 0; i < nData; i++) {
    ...
    if (e != thisLevelEnergy) {
      ...
      // DJ 8/29/06  Added E_1stExcitedState to G4MOSSNuclearLevel
      ...
      G4MOSSNuclearLevel* newLevel = new G4MOSSNuclearLevel(jlevel,
              _nucleusZ,
              _nucleusA,
              E_1stExcitedState,
              thisLevelEnergy,
  ...
  G4PtrSort<G4MOSSNuclearLevel>(_levels);
...
void G4MOSSNuclearLevelManager::PrintAll() {
  G4int nLevels = 0;
  if (_levels != 0) nLevels = _levels->size();

  G4cout << " ==== G4MOSSNuclearLevelManager ==== (" << _nucleusZ << ", " << _nucleusA
...
void G4MOSSNuclearLevelManager::PrintAllTabular(ofstream& file) {
  G4int nLevels = 0;
  if (_levels != 0) nLevels = _levels->size();

  for (int i = 0; i < nLevels; i++) {
    double TDebye = G4MOSSNuclearLevelStore::GetInstance()->GetManager(_nucleusZ, _nucleusA)->GetTDebye();
    _levels->operator[](i)->PrintAllTabular(file, _gsAngularMomentum, TDebye);
  }
}

void G4MOSSNuclearLevelManager::PrintLevelEnergies() {
...
G4MOSSNuclearLevelManager::G4MOSSNuclearLevelManager(const G4MOSSNuclearLevelManager &right) {
  ...
  if (right._levels != 0) {
    ...
    G4int i;
    for (i = 0; i < n; i++)
      _levels->push_back(new G4MOSSNuclearLevel(*(right._levels->operator[](i))));
...
G4double G4MOSSNuclearLevelManager::TeffIntegrand(G4double t) const {
  return t*t*t * (1.0/(G4Exp(t) - 1.0) + 0.5);
}


G4double G4MOSSNuclearLevelManager::CalcTeff(G4double TDebye, G4double Treal) {
  G4Integrator<const G4MOSSNuclearLevelManager, G4double(G4MOSSNuclearLevelManager::*)(G4double) const> integrator;
  ...
  G4double sum = integrator.Simpson(this, &G4MOSSNuclearLevelManager::TeffIntegrand,
...
void G4MOSSNuclearLevelManager::SetTeff(G4double Teff) { _Teff = Teff; }


G4double G4MOSSNuclearLevelManager::GetTeff() { return _Teff; }
