...
//          variable $G4MOSSGAMMADATA.  These files include:
...
//
//            RegisterPhysics(new G4MOSSPhysics("NRF", use_xsec_tables,
...
// Description:
//
// G4MOSS.cc is the main Geant4 code for simulating nuclear resonance fluorescence
...
//
//
// Major updates to the G4MOSS class by Jayson Vavrek (2018):
//
...
//
// 4) Encapsulation of G4MOSS by the G4MOSSPhysics class, which allows inheritance
...
// ****************************************************************************************************
// constructor
G4MOSS::G4MOSS(const G4String& processName, G4bool Verbose_in, G4bool use_xsec_tables_in,
  G4bool use_xsec_integration_in, G4bool force_isotropic_in)
  : G4VDiscreteProcess(processName),
    Verbose(Verbose_in),
    use_xsec_tables(use_xsec_tables_in=false),
    use_xsec_integration(use_xsec_integration_in=true),
    force_isotropic_ang_corr(force_isotropic_in=true),
    fAtomDeexcitation(0) {
  if (Verbose) {
    G4cout << "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&" << G4endl;
    G4cout << "G4MOSS Constructor is being called. "      << G4endl;
  ...
  if (!fAtomDeexcitation){
       G4cout << G4endl;
       G4cout << "WARNING from G4MOSS " << G4endl;
  ...
  // print gamma info to a datafile and disable some error checking in G4MOSSNuclearLevelManager
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
void G4MOSS::PrintInfoDefinition() {
...
// returned as DBL_MAX.
G4double G4MOSS::GetMeanFreePath(const G4Track& aTrack,
  ...
  if (Verbose) {
    G4cout << "In G4MOSS::GetMeanFreePath." << G4endl;
  ...
  while ((jelm < NumberOfElements) && (!found_nearby_level)) {
    ...
    if (Verbose) {
      ...
      G4cout << "G4MOSS::GetMeanFreePath -- Atomic number density (#/cm^3): "
    ...
    while ((jisotope < num_isotopes) && (!found_nearby_level)) {
      ...
      if (Verbose) {
        G4cout << "G4MOSS::GetMeanFreePath -- Isotope " << pIsotope->GetName()
         << " Z= " << Z << " A= " << A << G4endl;
        G4cout << "Fetching level manager from G4NuclearLevelStore." << G4endl;
      }

      pNuclearLevelManager =
        G4MOSSNuclearLevelStore::GetInstance()->GetManager(Z, A);
      ...
      if (Verbose) {
        ...
        G4cout << "G4MOSSGetMeanFreePath -- Searching for nearby level to E_gamma= " << GammaEnergy
       << G4endl;
      }

      G4MOSSNuclearLevel* pLevel = pNuclearLevelManager->NearestLevelRecoilAbsorb(GammaEnergy, E_TOL);
      ...
      if (pLevel != NULL) {   // i.e., kinematics permit gamma excitation
        if (Verbose) {
            G4cout << "G4MOSSGetMeanFreePath -- Found level at energy "
        ...
        if (Verbose) {
          G4cout << "G4MOSS::GetMeanFreePath -- Fractional abundance: " << frac_abundance
                 << G4endl;
        }
        ...
        if (Verbose) {
          G4cout << "G4MOSS::GetMeanFreePath: sigma (cm^-1): " << sigma/(1.0/cm) << G4endl;
        }
      } else {
        if (Verbose) {
          G4cout << "G4MOSS::GetMeanFreePath -- Could not find nearby level." << G4endl;
        }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4double G4MOSS::NRF_xsec_calc_gaus(G4double GammaEnergy, const G4MOSSNuclearLevel* pLevel) {
  ...
  G4MOSSNuclearLevelManager *pManager = G4MOSSNuclearLevelStore::GetInstance()->GetManager(Z, A);
  ...
  if (Verbose) {
    ...
    G4cout << "------------------------------------"    << G4endl;
    G4cout << "In G4MOSS::NRF_xsec_calc_gaus.     "    << G4endl;
...
void G4MOSS::SetParam_x(G4double x) {param_x = x;}

G4double G4MOSS::GetParam_x() const {return param_x;}

void G4MOSS::SetParam_t(G4double t) {param_t = t;}

G4double G4MOSS::GetParam_t() const {return param_t;}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4double G4MOSS::NRF_xsec_calc(G4double GammaEnergy, G4MOSSNuclearLevel* pLevel,
  ...
  const G4double theRand = G4UniformRand();
  G4double E_r;
  if((Z==26)&&(A==57)){
    G4double recoillessRatio = 0.86;
    if(theRand<=recoillessRatio){
      E_r = E1;
    }
    else{
      E_r = E1 + E_recoil/2.0;
  ...
  G4double fac1 = 2.0 * rootPi * stat_fac;
  const G4double fac2 = hbarc*hbarc/E_r/E_r;
  G4double fac3 = Gamma_0r / Delta_eff;
  ...
  if (useTables) {
    ...
    if((Z==26)&&(A==57)){
      ...
      G4double recoillessRatio = 0.86;
      // //G4cout << "the conversion possibility is " << pManager->GetTotalCC() << G4endl;
      if(theRand<=recoillessRatio){
        G4double conversion = 8.62;

        fac1 = 4*rootPi*rootPi*stat_fac;
        fac3 = 1.0/(1.0+conversion);
        fac4 = Gamma_r*Gamma_r/((Gamma_r*Gamma_r)+4*(E-E_r)*(E-E_r));
        xsec = fac1 * fac2 * fac3 * fac4;
        //G4cout << "the xsec is " << xsec/barn <<G4endl;
        //G4cout << "Gamma_r " << E_r << ";Gamma_0r " << Gamma_0r/eV<<";Gamma_r " << Gamma_r/eV << G4endl;
      }
      else{
      ...
      //G4cout << "the conversion possibility is " << pManager->GetTotalCC() << G4endl;
      if(theRand<recoillessRatio){
        G4double conversion = 5.12;//, Gamma_0r = 5.140707179395663e-14;
        ...
        //G4cout << "Gamma_r " << E_r << ";Gamma_0r " << Gamma_0r/eV << G4endl;
      }
      else{
        fac4 = PsiIntegral(x, teff, nMeshpoints, sigmaBound);
        xsec = fac1 * fac2 * fac3 * fac4;
  ...
  if (Verbose || (interrupt && xsec/barn > 1.0)) {
    G4cout << std::setprecision(12);
    G4cout << "------------------------------------"    << G4endl;
    G4cout << "In G4MOSS::NRF_xsec_calc.            "    << G4endl;
    G4cout << "Z:                  " << Z               << G4endl;
    G4cout << "A:                  " << A               << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4VParticleChange* G4MOSS::PostStepDoIt(const G4Track& trackData,
                                       const G4Step&  stepData) {
  aParticleChange.Initialize(trackData);

  const G4DynamicParticle* aDynamicParticle = trackData.GetDynamicParticle();
  G4double KineticEnergy = aDynamicParticle->GetKineticEnergy();
  G4ParticleMomentum photonDirection = aDynamicParticle->GetMomentumDirection();
  ...
  if (Verbose) {
    G4cout << "In G4MOSS::PostStepDoIt." << G4endl;
  ...
  pNuclearLevelManager = G4MOSSNuclearLevelStore::GetInstance()->GetManager(Z_excited, A_excited);

  if (pNuclearLevelManager) {
    const G4MOSSNuclearLevel* pLevel = pLevel_excited;

    if (!pLevel) {
      G4cout << "ERROR in G4MOSS::PostStepDoIt." << G4endl;
    ...
    while (continue_cascade) {
      ...
      if (Verbose) {
        G4cout << "G4MOSS::PostStepDoIt -- " << G4endl;
      ...
      // (1) Select gamma.  Note: G4MOSSLevel reports gamma energy as negative
      ...
      const G4MOSSNuclearLevel* pLevel_next = NULL;
      ...
      if (gamma_emission) {    // i.e. gamma emission, not conversion electron
        ...
        if (Verbose) {
          G4cout << "G4MOSS::PostStepDoIt -- Creating new gamma, direction = " << emitted_gamma_direction << G4endl;
        }
        ...
        //secParticles.push_back(aGamma);
      }  // gamma emission? (i.e. E_gamma > 0?)


      // //inner conversion electron
      if(E_gamma<0){
        //G4ThreeVector emitted_electron_direction = SampleIsotropic();
        G4double E_electron = -1*E_gamma;
        //G4DynamicParticle* aElectron = new G4DynamicParticle(G4Electron::Electron(), emitted_electron_direction, E_electron);
        //aParticleChange.AddSecondary(aElectron);

        const G4AtomicTransitionManager* transitionManager = G4AtomicTransitionManager::Instance();

        G4double kPoss = pNuclearLevelManager->GetTotalCC(), criticalVal = G4UniformRand();
        //G4cout << "the conversion ratio is " << kPoss << G4endl;

        size_t shellIndex = 0;

        if(criticalVal<kPoss){
          shellIndex = 0;
          //G4cout << "the vacancy is K!" << G4endl;
        }
        else{
          shellIndex = 1;
          //G4cout << "the vacancy is L!" << G4endl;
        }


        //size_t shellIndex = 0;
        const G4AtomicShell* shell = fTransitionManager->Shell(Z_excited,shellIndex);
        G4double bindingEnergy = shell->BindingEnergy();

        //G4cout << "the binding energy is " << bindingEnergy << G4endl;

        G4double localEnergyDeposit = 0.0;
        G4double cosTheta = 1.0;

        G4double eKineticEnergy = E_electron - bindingEnergy;
        // G4cout << "the element is " << Z_excited << ", "
        //        << "the ekinetic energy is " << bindingEnergy << G4endl
        //        << "the conversion electron energy is " << eKineticEnergy << G4endl;
        if (eKineticEnergy > 0.){
           ...
           // G4DynamicParticle* aElectron = new G4DynamicParticle(G4Electron::Electron(), emitted_electron_direction, E_electron);
            ...
            //secParticles.push_back(electron);
            //fvect->push_back(electron);
          }
          ...
          G4int index = trackData.GetMaterial()->GetIndex();
          ...
          {      
            ...
            //G4cout << "the previous fvect number is" << int(fvect->size()) << G4endl;

            fAtomDeexcitation->GenerateParticles(&secParticles,shell,Z_excited,index);

// //////////////////////////////////////////////////////////////////////////
// //////////////////////////////////////////////////////////////////////////
// /////  adding the deexcitation secondary particles
// ///////////////////////////////////////////////////////////////////////////
// ///////////////////////////////////////////////////////////////////////////

            G4int num = secParticles.size();
            // G4cout << "the secondary for deexcitation is " << num << G4endl;
            ...
            for (G4int i=0; i<num; ++i){
              if (secParticles[i]){
                ...
                //  G4cout << "added secondary particles! the particle is " << particlename 
                //         << ". the energy is " << dp->GetTotalEnergy() << G4endl;
...
// // /////////////////////////////////////////////////////////////////////////////
// // /////////////////////////////////////////////////////////////////////////////
// // ///// testing the fluo or auger 
// // //////////////////////////////////////////////////////////////////////
// // //////////////////////////////////////////////////////////////////////

// //             //G4cout << "the material index is " << index << G4endl;
// //             //G4cout << "the after fvect number is " << fvect->size() << G4endl;
...
// //           }
  ...
  } else { // pNuclearLevelManager is NULL -- anomalous condition, shouldn't have occurred if G4MOSS::GetMeanFreePath() behaved
    G4cout << "ERROR in G4MOSS::PostStepDoIT." << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
void G4MOSS::SetupMultipolarityInfo(const G4int nLevel,
           const G4double E_gamma, const G4int jgamma,
           const G4MOSSNuclearLevel* pLevel,
           const G4MOSSNuclearLevel* pLevel_next,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
void G4MOSS::AssignMultipoles(const G4MOSSNuclearLevel* pLevel,
           const G4double E_gamma, const G4int jgamma,
  ...
  if (Verbose) {
    G4cout << "G4MOSS::AssignMultipoles:" << G4endl;
  ...
  default:
    ...
    G4cout << "ERROR in G4MOSS::AssignMultipoles." << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4int G4MOSS::FindMin_L(const G4double Ji, const G4double Pi,
                       const G4double Jf, const G4double Pf,
                       char& transition) {
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4double G4MOSS::MixingRatio_WeisskopfEstimate(char transition, const G4int L,
  ...
  if (Verbose) {
    G4cout << "In G4MOSS::MixingRatio_WeisskopfEstimate." << G4endl;
    G4cout << "Input:" << G4endl;
  ...
  case 'O':
    ...
    break;
  default:
    G4cout << "Error in G4MOSS::MixingRatio_WeisskopfEstimate." << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4double G4MOSS::Lamda_Weisskopf(char multipole, const G4int L,
        const G4double E_gamma) {
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4ThreeVector G4MOSS::SampleCorrelation(const G4double Ji, const G4double J,  const G4double Jf,
  ...
  if (Verbose) {
    G4cout << "In G4MOSS::SampleCorrelation:" << G4endl;
    G4cout << "   Ji     = " << Ji << G4endl;
    G4cout << "   J      = " << J  << G4endl;
    G4cout << "   Jf     = " << Jf << G4endl;
    G4cout << "   L1     = " << L1 << G4endl;
  ...
  if (correlation_info_available) {
    ...
    if (Verbose) {
      G4cout << " G4MOSS::SampleCorrelation --" << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4ThreeVector G4MOSS::SampleIsotropic() {
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
// ****************************************************************************************************
G4bool G4MOSS::ForbiddenTransition(const G4MOSSNuclearLevel* pLevel,
          const G4MOSSNuclearLevel* pLevel_next) {
...
G4double G4MOSS::expIntegrand(G4double y) const {
...
G4double G4MOSS::PsiIntegral(G4double x, G4double t, G4int nMeshpoints, G4double sigmaBound) {
  ...
  G4double sum = integrator.Simpson(this, &G4MOSS::expIntegrand, theLowerLimit, theUpperLimit, nMeshpoints);

  return sum;
}

void G4MOSS::MakeCrossSectionTable(G4MOSSNuclearLevel* pLevel, G4double Teff) {
  G4int A = pLevel->A();
  G4int Z = pLevel->Z();
...
G4double G4MOSS::InterpolateCrossSection(const G4MOSSNuclearLevel* pLevel, G4double GammaEnergy) {
...
void G4MOSS::print_to_standalone(ofstream& file) {
  G4cout << G4endl;
  G4cout << "Calling G4MOSS::print_to_standalone()" << G4endl;

  for (int z = 4; z <= 100; ++z) {
    for (int a = z; a <= 3*z; ++a) {
      G4MOSSNuclearLevelManager* pManager = G4MOSSNuclearLevelStore::GetInstance()->GetManager(z, a, true);
...
G4double G4MOSS::SampleElectronDirection(G4double energy){
  G4double costheta = 1.0;
