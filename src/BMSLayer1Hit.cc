...
//
//
/// \file BMSLayer1Hit.cc
/// \brief Implementation of the BMSLayer1Hit class
...
G4ThreadLocal G4Allocator<BMSLayer1Hit>* BMSLayer1HitAllocator;

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

BMSLayer1Hit::BMSLayer1Hit()
: G4VHit(), 
   fEdep(0),
   fTime(0),
   fScatteringPos(0),
...
//BMSLayer1Hit::BMSLayer1Hit(G4int layerID)
//: G4VHit(),
...
BMSLayer1Hit::BMSLayer1Hit(const BMSLayer1Hit &right)
: G4VHit(),
  fEdep(right.fEdep),
  fTime(right.fTime),
...
const BMSLayer1Hit& BMSLayer1Hit::operator=(const BMSLayer1Hit &right)
{
  fEdep = right.fEdep;
  fTime = right.fTime;
  fScatteringPos = right.fScatteringPos;
...
G4bool BMSLayer1Hit::operator==(const BMSLayer1Hit &/*right*/) const
{
  return false;
}
...
const std::map<G4String,G4AttDef>* BMSLayer1Hit::GetAttDefs() const
{
  G4bool isNew;
  auto store = G4AttDefStore::GetInstance("BMSLayer1Hit",isNew);
...
std::vector<G4AttValue>* BMSLayer1Hit::CreateAttValues() const
{
  auto values = new std::vector<G4AttValue>;
  
  values
    ->push_back(G4AttValue("HitType","BMSLayer1Hit",""));
  values
