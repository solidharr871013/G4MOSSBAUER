...
//
//
// $Id: G4MOSS.hh,v 1.1.1.1 2007/01/13 19:44:02 jordan Exp $
// GEANT4 tag $Name:  $
//
//      ------------ G4MOSS physics process --------
//
// G4MOSS Class description
//
...
class G4MOSS : public G4VDiscreteProcess {
 public:
  G4MOSS(const G4String& processName = "NRF", G4bool Verbose_in = false,
  ...
  G4double InterpolateCrossSection(const G4MOSSNuclearLevel* pLevel, G4double GammaEnergy);

  void print_to_standalone(ofstream& file);

 private:
  G4MOSS & operator=(const G4MOSS &right);
  G4MOSS(const G4MOSS&);
  G4MOSSNuclearLevelManager* pNuclearLevelManager;
  G4double NRF_xsec_calc_gaus(G4double GammaEnergy, const G4MOSSNuclearLevel* pLevel);
  G4double NRF_xsec_calc(G4double GammaEnergy, G4MOSSNuclearLevel* pLevel,
    G4bool useTables, G4int nMeshpoints = 300, G4double sigmaBound = 4.0);
  void MakeCrossSectionTable(G4MOSSNuclearLevel *pLevel, G4double Teff);
  ...
  void AssignMultipoles(const G4MOSSNuclearLevel* pLevel,
      const G4double E_gamma,
      const G4int jgamma,
  ...
  G4bool ForbiddenTransition(const G4MOSSNuclearLevel* pLevel,
    const G4MOSSNuclearLevel* pLevel_next);
  ...
  G4int A_excited;
  G4int Z_excited;
  const G4MOSSNuclearLevel* pLevel_excited;
  ...
  G4Integrator<const G4MOSS, G4double(G4MOSS::*)(G4double) const> integrator;
};

inline G4bool G4MOSS::IsApplicable(const G4ParticleDefinition& particle) {
